...
const Messages = (props: MessagesProps) => {
    ...
    const { data, isLoading, mutate } = useSWR<Messages>('/api/guestbook', fetcher)
                    ...
                    <div key={id} className="rounded-lg border border-accent-2 p-4">
                        <div className="mb-3 flex gap-3">
                            ...
                            <div className="flex flex-col justify-center gap-px text-sm">
                                <div>{created_by}</div>
                                <div className="text-xs text-accent-5">{dayjs(updated_at).format('YYYY-MM-DD')}</div>
                            </div>
                        </div>
                            ...
                            <div className="mt-4 flex justify-end">
                                <AlertDialog>
                                    <AlertDialogTrigger asChild>
                                        <Button disabled={isDeleting} variant="danger" type="button">
                                            Delete
                                        </Button>
                                    </AlertDialogTrigger>
                                    <AlertDialogContent>

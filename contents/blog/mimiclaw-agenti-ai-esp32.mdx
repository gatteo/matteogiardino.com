...
authorId: matteo
locale: 'it'
tags: ['openclaw', 'ai', 'hardware', 'ai agents']
translationSlug: 'mimiclaw-openclaw-esp32'
---
...
<Mermaid caption="Divisione dual-core ESP32-S3 - rete su Core 0, AI su Core 1, PSRAM e flash condivise">
{`
flowchart LR
  subgraph C0["Core 0 - Rete"]
    WIFI["Polling Wi-Fi"]
    TG["I/O Telegram"]
  end
  subgraph C1["Core 1 - AI"]
    AGENT["Loop agente"]
    LLM["Chiamate LLM"]
    TOOLS["Esecuzione tool"]
  end
  subgraph SHARED["Risorse condivise"]
    PSRAM["8 MB PSRAM"]
    FLASH["16 MB flash"]
  end
  C0 --- SHARED
...
<Mermaid caption="Divisione memoria MimiClaw - lavoro veloce e volatile in PSRAM, stato persistente in flash">
{`
flowchart LR
  subgraph PSRAM["8 MB PSRAM - working set"]
    LLMBUF["Buffer stream LLM"]
    CTX["Buffer contesto<br/>cronologia conversazioni"]
    MSG["Buffer messaggi<br/>I/O Telegram"]
  end
  subgraph FLASH["16 MB flash - persistente"]
    HIST["Cronologia conversazioni"]
    MEM["File di memoria"]
    SKILLS["Skill"]
    CRON["Cron job"]
  end
...
<Mermaid caption="Layout partizioni flash - credenziali in NVS, slot firmware A/B per OTA, file dell'agente in SPIFFS">
{`
flowchart LR
  subgraph FLASH["Layout partizioni flash 16 MB"]
    NVS["NVS · 24 KB<br/>credenziali, token"]
    APP0["app0 · 3 MB<br/>firmware primario"]
    APP1["app1 · 3 MB<br/>slot aggiornamento OTA"]
    SPIFFS["SPIFFS · 9+ MB<br/>skill, memoria, cron"]
  end
`}
</Mermaid>

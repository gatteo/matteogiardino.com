...
Security is paramount when agents make external API calls. The Envoy plugin forces all outbound traffic from OpenClaw through a strict proxy allowlist. For a complete security overview, check out our guide on [OpenClaw MCP Envoy Egress Proxy Allowlist Config](/en/blog/openclaw-mcp-envoy-egress-proxy-allowlist-config). If a hijacked prompt tries to exfiltrate data, the proxy blocks it.

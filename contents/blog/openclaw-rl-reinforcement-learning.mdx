...
translationSlug: 'openclaw-rl-apprendimento-rinforzo'
---
...
<Mermaid caption="OpenClaw-RL: chat traffic and gradient updates run in parallel without blocking each other">
{`
flowchart LR
  Client["OpenClaw client<br/>your assistant"]
  Model["Model server<br/>OpenAI-compatible :30000"]
  PRM["PRM server<br/>process reward model"]
  Train["Training engine<br/>gradient updates"]

  Client <-->|conversations| Model
  Model -->|each turn| PRM
  PRM -->|scalar reward| Train
  Train -.->|new weights| Model
`}
</Mermaid>

...
<Mermaid caption="Local RAG pipeline - documents become embeddings once, then user questions retrieve and synthesize entirely on-prem">
{`
flowchart LR
  DOCS["Documents<br/>PDF · MD · txt"]
  INGEST["Ingestion script<br/>chunk + embed"]
  STORE[("Vector store<br/>ChromaDB / FAISS")]
  USER["User question"]
  AGENT["Retrieval agent"]
  LLM["Local LLM<br/>synthesis"]
  RESP["Answer"]

  DOCS --> INGEST --> STORE
  USER --> AGENT
  AGENT -->|query| STORE
  STORE -->|relevant chunks| AGENT
  AGENT --> LLM --> RESP
`}
</Mermaid>

...
UI-TARS è un agente AI visivo open source basato su Qwen 2 VL in grado di controllare nativamente sia il tuo browser che l'ambiente desktop. Elabora i pixel grezzi dello schermo per cliccare, digitare e navigare le interfacce grafiche in totale autonomia, senza dipendere dal parsing HTML o da costose API esterne. Essendo eseguibile localmente, offre una soluzione gratuita e privata per automatizzare flussi di lavoro web e di sistema complessi.

Se stai cercando un modo affidabile per automatizzare le tue attività, UI-TARS cambia le regole del gioco. Costruito su eccellenti fondamenta visive, è un agente GUI specializzato capace di "guardare" il tuo schermo ed eseguire task complessi in completa autonomia. In questa guida, analizzeremo cosa rende speciale UI-TARS, le sue funzionalità principali e come puoi eseguirlo direttamente sul tuo hardware.
...
Al momento, gli agenti visivi tendono ad aver bisogno del focus sullo schermo reale o virtuale per "vedere" cosa succede, a differenza delle automazioni API-first. L'utilizzo tramite container Docker (come negli HuggingFace Spaces) è la soluzione ideale per task in background.

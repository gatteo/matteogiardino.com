...
translationSlug: 'delegate-coding-tasks-openclaw-agents-acp'
---
...
<Mermaid caption="Handshake ACP - l'agente principale resta libero mentre il sub-agente di coding lavora in isolamento">
{`
sequenceDiagram
  autonumber
  participant Parent as Agente padre
  participant ACP as Layer ACP
  participant Sub as Sub-agente coding
  Parent->>ACP: sessions_spawn(task, runtime: acp)
  ACP->>Sub: handshake + workspace
  Sub-->>ACP: pronto
  ACP-->>Parent: agent_id
  loop fino a completamento
    Sub->>Sub: leggi · pianifica · modifica
    Sub->>ACP: tool_call(write / run / test)
    ACP-->>Sub: risultato
  end
  Sub->>ACP: complete(diff, sommario)
  ACP-->>Parent: risultati
`}
</Mermaid>

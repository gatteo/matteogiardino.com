...
translationSlug: 'mimiclaw-agenti-ai-esp32'
---
...
<Mermaid caption="ESP32-S3 dual-core split - network on Core 0, AI on Core 1, shared PSRAM and flash">
{`
flowchart LR
  subgraph C0["Core 0 - Network"]
    WIFI["Wi-Fi polling"]
    TG["Telegram I/O"]
  end
...
<Mermaid caption="MimiClaw memory split - fast volatile work in PSRAM, persistent state in flash">
{`
flowchart LR
  subgraph PSRAM["8 MB PSRAM - working set"]
    LLMBUF["LLM stream buffer"]
    CTX["Context buffer<br/>conversation history"]
    MSG["Message buffers<br/>Telegram I/O"]
  end
  subgraph FLASH["16 MB flash - persistent"]
...
<Mermaid caption="Flash partition layout - credentials in NVS, A/B firmware slots for OTA, agent files in SPIFFS">
{`
flowchart LR
  subgraph FLASH["16 MB flash partition layout"]
    NVS["NVS · 24 KB<br/>credentials, tokens"]
    APP0["app0 · 3 MB<br/>primary firmware"]
    APP1["app1 · 3 MB<br/>OTA update slot"]
    SPIFFS["SPIFFS · 9+ MB<br/>skills, memory, cron"]
  end
`}
</Mermaid>

...
![Installazione di QVAC SDK in una sandbox pulita su macOS per la trascrizione offline](/images/blog/qvac-whisper-offline-transcription-macos/01-install.png)
...
- `modelType: "whispercpp-transcription"` è la literal che lo schema dell'SDK richiede. Vecchie docs e snippet del blog QVAC a volte mostrano `modelType: "whisper"` — è sbagliato rispetto allo schema attuale e tira fuori un errore di validazione. Usa la forma lunga.
...
![Output del primo run: WHISPER_TINY trascrive il file WAV di esempio da disco su macOS](/images/blog/qvac-whisper-offline-transcription-macos/02-first-run.png)
...
![Benchmark di WHISPER_TINY vs WHISPER_EN_BASE_Q8_0 su un clip podcast da 35 secondi su macOS](/images/blog/qvac-whisper-offline-transcription-macos/03-benchmark.png)
...
![CLI transcribe in esecuzione con il flag --accurate, più i percorsi di errore per argomenti mancanti e file mancanti](/images/blog/qvac-whisper-offline-transcription-macos/04-cli.png)
...
- **`error: invalid model type`.** Hai scritto `modelType: "whisper"` invece di `modelType: "whispercpp-transcription"`. Lo schema Zod rifiuta la forma corta. Usa la literal lunga — è il nome dell'engine, non il nome friendly della capability.

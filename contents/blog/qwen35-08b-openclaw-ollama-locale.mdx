...
summary: 'Scopri come eseguire qwen 3.5 0.8b in locale con openclaw e ollama su cpu. Una guida completa per modelli da 800M parametri senza GPU.'
image: '/images/blog/qwen35-08b-openclaw-ollama-locale.png'
authorId: matteo
locale: 'it'
tags: ['ai', 'openclaw', 'llm']
translationSlug: 'run-qwen35-08b-openclaw-ollama-cpu'
---

Ho installato e configurato Qwen 3.5 0.8B con [OpenClaw](/blog/guida-cio-cto-forza-lavoro-agenti-ai-openclaw) e Ollama su Ubuntu, tutto su CPU. Nessuna GPU coinvolta. Il modello pesa circa 1 GB e risponde via Telegram in pochi secondi.

Se ti stai chiedendo come eseguire qwen 3.5 0.8b in locale con openclaw e ollama su cpu, sei nel posto giusto. Questa guida esplora in dettaglio le prestazioni di questo modello da 800 milioni di parametri in contesti a basse risorse, fornendo metriche reali sul mio server domestico. Il setup funziona su qualsiasi macchina recente con CPU decente, ideale per test, sviluppo o deployment su VPS economici.
...
### Come eseguire Qwen 3.5 0.8B in locale con OpenClaw e Ollama su CPU?
Basta installare Ollama, scaricare il modello tramite il comando `ollama pull qwen3.5:0.8b` e impostare il provider `ollama` nel file di configurazione `config.yaml` di OpenClaw. Questo setup funziona interamente su processori tradizionali senza richiedere schede video dedicate.

### Qual è la latenza media su un sistema senza GPU?
Le prestazioni variano in base al processore. Su un Mac Mini con chip M4, i tempi di risposta medi si aggirano intorno ai 2-3 secondi, mentre su un server Linux virtuale di fascia economica (VPS) i tempi arrivano a circa 5-8 secondi.

### Quanta memoria RAM serve per Qwen 3.5 0.8B?
Il modello Qwen 3.5 0.8B è estremamente leggero e pesa solo circa 1 GB. Con l'aggiunta di OpenClaw e Ollama, è possibile far girare l'intero ambiente in modo stabile con appena 2 GB di RAM libera, risultando ideale per Raspberry Pi o vecchi portatili.

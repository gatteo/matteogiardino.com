...
Basta installare Ollama, tirare il modello (`ollama pull qwen3.5:0.8b`) e configurare il provider `ollama` in `config.yaml` di OpenClaw. Gira tutto su CPU senza problemi.

**Qual è la latenza media su un sistema senza GPU?**
Dipende dal processore. Su un Mac Mini M4, parliamo di 2-3 secondi, su un server economico Linux circa 5-8 secondi.

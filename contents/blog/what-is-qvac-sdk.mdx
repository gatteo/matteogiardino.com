...
The local-AI ecosystem has been picking sides for two years. `llama.cpp` went deep on inference. Ollama went deep on developer ergonomics for desktop chat. MLX went deep on Apple Silicon. `transformers.js` went deep on the browser. QVAC is the first credible attempt to **go wide instead** - multi-modality, multi-runtime, multi-platform, with P2P plumbing as a bonus. If you've watched [the move from chatbots to agents](/blog/chatbots-to-agents-2026) and noticed how quickly real products need transcription + RAG + tool-use + image generation in one app, you can see why an SDK-shaped answer was inevitable.
